...
GlideLDAP API is undocumented and an unsupported API method to query the LDAP with reliance on VPN / Direct connectivity to LDAP between ServiceNow and Customer Network to query the LDAP. Rather use Data Sources along with MID Server to query the LDAP where needed./
